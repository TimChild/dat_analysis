...
################# Other configurables #########################
ddir = os.path.join('./Data/Experiment_Data')
pickledata = os.path.join('./Data/Pickles')
plotdir = os.path.join('./Data/Plots')
dfdir = os.path.join('./Data/DataFrames')
...
if platform.system() == "Darwin":  # Darwin is Mac... So I can work with Owen
    ...
    os.chdir('D:/OneDrive/UBC LAB/GitHub/Python/OneCK/')
...
class DatPD(object):  # FIXME: OWEN, have I built the singleton correctly?
    ...
    def __new__(cls, **kwargs):
        if 'name' in kwargs.keys() and kwargs['name'] is not None:
            name = kwargs['name']
            datPDpath = os.path.join(dfdir, f'{name}.pkl')
        else:
            datPDpath = os.path.join(dfdir,
    ...
    def save(self, name=None):
        if name is not None:
            ...
            datPDpath = os.path.join(dfdir, f'default.pkl')
