...
def power_spectrum(data, meas_freq, normalization=1, ax=None, **kwargs):
    if ax is None:
        ...
        ax: plt.Axes
    freq, power = dat_analysis.useful_functions.power_spectrum(data, meas_freq, normalization)
...
def display_1d(x: np.array, data: np.array, ax: plt.Axes = None, x_label: str = None, y_label: str = None, errors: np.array = None, auto_bin=True, **kwargs):
    ...
    dat_analysis.Plotting.Mpl.PlotUtil.del_kwarg(['swap_ax', 'scatter', 'label', 'linewidth', 'marker', 'color'], kwargs)
