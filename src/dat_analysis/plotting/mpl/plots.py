...
from dat_analysis.plotting.mpl.util import (
    xy_to_meshgrid,
    addcolorlegend,
    make_axes,
    ax_setup,
...
def power_spectrum(data, meas_freq, normalization=1, ax=None, **kwargs):
    if ax is None:
        ...
        ax: plt.Axes
    freq, power = dat_analysis.useful_functions.power_spectrum(
        data, meas_freq, normalization
    )
...
def display_2d(
    ...
    **kwargs,
    ...
    if (
        x_label is not None and kwargs.get("x_label", None) is None
    ):  # kwargs gets precedence
...
def display_1d(
    ...
    **kwargs,
    ...
    if errors is not None:
        errors = [
            np.nan if e in [None] else e for e in errors
        ]  # fix errors if None values given...
    ...
    dat_analysis.Plotting.Mpl.PlotUtil.del_kwarg(
        ["swap_ax", "scatter", "label", "linewidth", "marker", "color"], kwargs
    )
...
def df_table(df: pd.DataFrame, title=None, sig_fig=3):
    ...
    if title is not None:
        ax.set_title(title)
    table = ax.table(
        cellText=(CU.sig_fig(df, sig_fig)).values,
...
def waterfall_plot(
    ...
    def get_plot_fn(ptype):
        ...
        if ptype == "plot":
            ...
            print(
                "ERROR[waterfall_plot]: Need to specify ptype (plot_type, e.g. plot, scatter, error)"
            )
    ...
    if {"color", "c"} & set(
        plot_args.keys()
    ):  # Needed for how I deal with colors in plotting
    ...
    def get_colors_list(color, num):
        ...
        if color is None:
            ...
            print(
                f"WARNING[waterfall_plot]: Color must be a str or np.ndarray with len(y)"
            )
            return get_colors(num, cmap_name=cmap_name)
    ...
    if x.ndim == data.ndim:
        ...
        raise ValueError(
            f"ERROR[PF.waterfall_plot]: Wrong shape of x, y passed. Got ([{x.shape}], [{data.shape}] "
