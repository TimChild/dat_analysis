...
class DataAligner:
    ...
    def fit_translation(self, x, y, data, params=None, method="powell"):
        ...
        for ax, arrs in zip(["x", "y"], [(self.x_match, x), (self.y_match, y)]):
            smaller_data_width = min(
                [abs(np.nanmax(_a) - np.nanmin(_a)) for _a in arrs]
            )
            _min = params[f"{ax}_translate"].min
            params[f"{ax}_translate"].min = (
                _min if _min != -np.inf else -smaller_data_width
            )
            _max = params[f"{ax}_translate"].max
            params[f"{ax}_translate"].max = (
                _max if _max != np.inf else smaller_data_width
            )
    ...
    def translate_data_to_match(self, x, y, data, params=None, method="powell"):
        ...
        xt, yt, datat = translate_data(
            x,
            y,
            data,
            self.last_fit.params["x_translate"].value,
    ...
    def subtract_data(self, x, y, data, translate_first=False, invert=False):
        ...
        nx, ny, diff = subtract_data(
            self.x_match, self.y_match, self.data_match, x, y, data
        )
        if invert:
            diff *= -1
        return nx, ny, diff
...
def subtract_data(x1, y1, data1, x2, y2, data2):
    ...
    if not np.all(x1 == x2):
        nx = np.linspace(
            max(np.nanmin(x1), np.nanmin(x2)),
            min(np.nanmax(x1), np.nanmax(x2)),
            max(x1.shape[-1], x2.shape[-1]),
        )
    else:
        nx = x1
    if not np.all(y1 == y2):
        ny = np.linspace(
            max(np.nanmin(y1), np.nanmin(y2)),
            min(np.nanmax(y1), np.nanmax(y2)),
            max(y1.shape[-1], y2.shape[-1]),
        )
    else:
        ny = y1
...
class DataAligner1D:
    ...
    def fit_translation(self, x, data, params=None, method="powell", max_shift=None):
        ...
        if max_shift is None:
            max_shift = min(
                [abs(np.nanmax(_x) - np.nanmin(_x)) for _x in [x, self.x_match]]
            )
        ...
        fit = lm.minimize(
            ...
            params,
            method=method,
            args=[self.x_match, self.data_match, x, data],
        )
        self.last_fit = fit
        return fit

    def translate_data_to_match(
        self, x, data, params=None, method="powell", max_shift=None
    ):
        """Translate data to minimize difference"""
        fit = self.fit_translation(
            x, data, params=params, method=method, max_shift=max_shift
        )
        xt, datat = translate_data_1d(
            x, data, self.last_fit.params["x_translate"].value
        )
        return xt, datat

    def subtract_data(
        self, x, data, translate_first=False, invert=False, max_shift=None
    ):
        """Subtract provided data from match data"""
        if translate_first:
