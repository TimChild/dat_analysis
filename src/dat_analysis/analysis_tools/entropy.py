...
def get_param_estimates(x_array, data, mids=None, thetas=None) -> List[lm.Parameters]:
    if data.ndim == 1:
        ...
        return [
            _get_param_estimates_1d(x_array, z, mid, theta)
            for z, mid, theta in zip(data, mids, thetas)
        ]
...
def fit_entropy_data(
    x,
    z,
    params: Optional[Union[List[lm.Parameters], lm.Parameters]] = None,
    auto_bin=False,
):
    if params is None:
    ...
    if z.ndim == 1:  # 1D data
        ...
        for i in range(z.shape[0]):
            fit_result_list.append(
                fit_entropy_1d(x, z[i, :], params[i], auto_bin=auto_bin)
            )
...
class EntropySignalProcess(Process):
    ...
    def get_output_plotter(
        ...
        data_label: str = f"{DELTA} Current /nA",
