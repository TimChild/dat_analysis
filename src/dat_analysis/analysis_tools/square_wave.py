...
def get_setpoint_indexes_from_times(
    dat: Any, start_time: Optional[float] = None, end_time: Optional[float] = None
...
@deprecated(
    deprecated_in="3.2.0",
    details="Un-deprecate if used in new subclass of Data, but otherwise remove",
)
...
def get_transition_parts(part: Union[str, int]) -> Union[tuple, int]:
    if isinstance(part, str):
        ...
        if part == "cold":
            ...
            raise ValueError(
                f'{part} not recognized. Should be in ["hot", "cold", "vp", "vm"]'
            )
...
@deprecated(
    deprecated_in="3.0.0",
    details="was used for old dat. OK to undeprecate if it becomes useful again",
)
...
class SeparateSquareProcess(Process):
    ...
    def _preprocess(self):
        ...
        data_by_setpoint = i_sense.reshape(
            (i_sense.shape[0], -1, 4, self.inputs["samples_per_setpoint"])
        )

        delay_index = round(
            self.inputs["setpoint_average_delay"] * self.inputs["measure_freq"]
        )
        if delay_index > self.inputs["samples_per_setpoint"]:
            setpoint_duration = (
                self.inputs["samples_per_setpoint"] / self.inputs["measure_freq"]
            )
            raise ValueError(
                f'setpoint_average_delay ({self.inputs["setpoint_average_delay"]} s) is longer than '
    ...
    def process(self):
        self._preprocess()
        separated = np.mean(
            self._preprocessed["data_by_setpoint"][
                :, :, :, self._preprocessed["delay_index"] :
            ],
            axis=-1,
        )
    ...
    def get_input_plotter(
        ...
        end_x: Optional[float] = None,  # To only average between
        y: Optional[np.ndarray] = None,
        start_y: Optional[float] = None,
        end_y: Optional[float] = None,  # To only average between
    ) -> DataPlotter:
        self._preprocess()
        ...
        if start_y or end_y:
            ...
            by_setpoint = by_setpoint[
                s_
            ]  # slice of rows, all_dac steps, 4 parts, all datapoints
